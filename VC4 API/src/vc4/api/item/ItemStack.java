...
public class ItemStack implements Comparable<ItemStack>, Serializable {
	...
	/*
	 * public String getArmourSlotText(int slot){ String stats; if(getStats().getVanity()){ stats = getStatsText(); } else if(ContainerArmour.isVanitySlot(slot)){ stats = "\r\n{c:3}" + Localization.getLocalization("item.invanity"); } else {
	 * stats = getStatsText(); } StringBuilder text = new StringBuilder(); text.append(getName() + "{c:f}"); if(getItem().isDamagedOnUse){ double d = 1D - (getDamage() / (double)getItem().maxDamage); Color c =
	 * ColorUtils.differColors(Color.red, Color.green, (float)d); String s = Integer.toHexString(c.getRGB()); int len = 6 - s.length(); for(int dofor = 0; dofor < len; ++dofor){ s = "0" + s; }
	 * text.append(" ({c:").append(s).append("}").append(getDamage()).append("/").append(getItem().maxDamage).append("�f)"); } text.append(getDescription()); text.append(stats); text.append(getEnchantmentText()); return text.toString(); }
	 */
