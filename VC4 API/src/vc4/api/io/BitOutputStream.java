 ...
 * to a stream (even though a byte has to be filled until the data is flushed to the underlying output stream). It is also able to write an integer value to the stream using the specified number of bits.
 * 
 * @author Andreas Jakl
 */
public class BitOutputStream implements AutoCloseable, BitOutput {
	/**
	...
	/**
	 * The temponary buffer containing the individual bits until a byte has been completed and can be commited to the output stream.
	 */
	private int iBuffer;
	...
	/**
	 * Write the specified number of bits from the int value to the stream. Correspondig to the InputStream, the bits are written starting at the highest bit ( >> aNumberOfBits ), going down to the lowest bit ( >> 0 ).
	 * 
	 * @param aValue
	 ...
	 * 
	 * @param aValue
	 *            the long containing the bits that should be written to the stream.
	 * @param aNumBits
	 *            how many bits of the integer should be written to the stream.
	 * @throws IOException
	 */
	@Override
	synchronized public void writeBits(final long aValue, final int aNumBits) throws IOException {
		for (int i = aNumBits - 1; i >= 0; i--) {
			writeBit(((aValue >> i) & 0x01));
		}
	}
	...
	/**
	 * Calculate how many bits are needed to store the specified value. Can be used to optimize data transfer.
	 * 
	 * @param aMaxValue
	...
	public void writeBytes(byte[] b) throws IOException {
		for (int dofor = 0; dofor < b.length; ++dofor)
			writeByte(b[dofor]);
	}
