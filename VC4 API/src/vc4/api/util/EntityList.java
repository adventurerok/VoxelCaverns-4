...
public class EntityList implements RandomAccess, Cloneable, java.io.Serializable {
	 ...
	 */
	public EntityList(Collection<Entity> c) {
		elementData = c.toArray(new Entity[c.size()]);
	 ...
	 */
	public boolean contains(Entity f) {
		return indexOf(f) >= 0;
	}
	 ...
	 * <tt>(o==null&nbsp;?&nbsp;get(i)==null&nbsp;:&nbsp;o.equals(get(i)))</tt>, or -1 if there is no such index.
	 */
	public int lastIndexOf(Entity f) {
	...
	public EntityList clone() {
		EntityList v = new EntityList(size);
	 ...
	 */
	public boolean remove(Entity f) {
	 ...
	 *             if the specified collection is null
	 */
	public boolean addAll(Collection<Entity> c) {
		Entity[] a = c.toArray(new Entity[c.size()]);
		int numNew = a.length;
	 ...
	 * ArrayIndexOutOfBoundsException if index is negative.
	 */
	private void RangeCheckThrows(int index) {
		if (index >= size)
			throw new IndexOutOfBoundsException("Index: " + index + ", Size: " + size);
	}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		...
		Entity[] a = elementData = new Entity[arrayLength];

		// Read in all elements in the proper order.
		for (int i = 0; i < size; i++)
			a[i] = (Entity) s.readObject();
	}
}
