...
public class SuidUtils {
	...
	public static byte[] parseSuid(String hex) {
		...
		for (int d = 0; d < 32; ++d) {
			if ((d % 2) == 0)
				current = (byte) (toHex(hex.charAt(d)) << 4);
			else {
	...
	private static byte toHex(char nibble) {
		...
		if (nibble > 47 && nibble < 58)
			return (byte) (nibble - 48);
		else
			return (byte) (nibble - 87);
	}
	...
	public static String getSuidPath(byte[] suid) {
		if (suid == null || suid.length != 16)
			throw new RuntimeException("SUID must be 128-bit integer");
