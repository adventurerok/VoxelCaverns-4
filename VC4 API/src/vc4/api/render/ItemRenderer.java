...
public class ItemRenderer {
    ...
    public static void renderItem3D(ItemStack stack, float x, float y, float z, Renderer render) {
        if (stack.isBlock() && Block.byId(stack.getId()).render3d(stack.getData())) {
            Block.byId(stack.getId()).getRenderer().renderBlock(stack, x - 0.5f, y - 0.5f, z - 0.5f, render);
        }
        Item s = stack.getItem();
        int tex = s.getTextureIndex(stack);
        render.color(s.getColor(stack));
        renderItemWithThickness(x, y, z, tex, render);
        if (s instanceof IItemMultitexture) {
            tex = ((IItemMultitexture) s).getMultitextureTextureIndex(stack);
            render.color(((IItemMultitexture) s).getMultitextureColor(stack));
            renderItemWithThickness(x, y, z, tex, render);
        }
    }
    ...
    protected static void renderItemWithThickness(float x, float y, float z, float tid, Renderer render) {
        x -= 0.5F;
        y -= 0.5F;
        z += 0.03125F;
        float f = x + 1.0F;
