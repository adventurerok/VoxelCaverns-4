...
public abstract class RecursiveGenerator {
	...
	protected abstract void generateRecursive(Random rand, long x, long y, long z, long cx, long cy, long cz, GeneratorOutput data);
		...
		for(long ax = x - range; ax <= x + range; ++ax){
			for(long ay = y - range; ay <= y + range; ++ay){
				for(long az = z - range; az <= z + range; ++az){
					...
					generateRecursive(rand, x, y, z, ax, ay, az, data);
				}
			}
		}
	}
