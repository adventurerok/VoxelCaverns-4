...
/**
 * An {@link Escaper} that converts literal text into a format safe for inclusion in a particular context (such as an XML document). Typically (but not always), the inverse process of "unescaping" the text is performed automatically by the
 * relevant parser.
 * 
 * <p>
 * For example, an XML escaper would convert the literal string {@code "Foo<Bar>"} into {@code "Foo&lt;Bar&gt;"} to prevent {@code "<Bar>"} from being confused with an XML tag. When the resulting XML document is parsed, the parser API will
 * return this text as the original literal string {@code "Foo<Bar>"}.
 * 
 * <p>
 * <b>Note:</b> This class is similar to {@link CharEscaper} but with one very important difference. A CharEscaper can only process Java <a href="http://en.wikipedia.org/wiki/UTF-16">UTF16</a> characters in isolation and may not cope when
 * it encounters surrogate pairs. This class facilitates the correct escaping of all Unicode characters.
 * 
 * <p>
 ...
 * 
 * <p>
 * Several popular escapers are defined as constants in the class {@link CharEscapers}. To create your own escapers extend this class and implement the {@link #escape(int)} method.
 * 
 * 
 */
public abstract class UnicodeEscaper implements Escaper {
	...
	/**
	 * Returns the escaped form of the given Unicode code point, or {@code null} if this code point does not need to be escaped. When called as part of an escaping operation, the given code point is guaranteed to be in the range
	 ...
	 * 
	 * <p>
	 * If the implementation of this method cannot correctly handle a particular code point then it should either throw an appropriate runtime exception or return a suitable replacement character. It must never silently discard invalid
	 * input as this may constitute a security risk.
	 * 
	 * @param cp
	...
	/**
	 * Scans a sub-sequence of characters from a given {@link CharSequence}, returning the index of the next character that requires escaping.
	 * 
	 * <p>
	 * <b>Note:</b> When implementing an escaper, it is a good idea to override this method for efficiency. The base class implementation determines successive Unicode code points and invokes {@link #escape(int)} for each of them. If the
	 * semantics of your escaper are such that code points in the supplementary range are either all escaped or all unescaped, this method can be implemented more efficiently using {@link CharSequence#charAt(int)}.
	 * 
	 * <p>
	 * Note however that if your escaper does not escape characters in the supplementary range, you should either continue to validate the correctness of any surrogate characters encountered or provide a clear warning to users that your
	 * escaper does not validate its input.
	 * 
	 * <p>
	 ...
	 * {@link IllegalArgumentException}. You should either ensure your input is valid <a href="http://en.wikipedia.org/wiki/UTF-16">UTF-16</a> before calling this method or use an escaped {@link Appendable} (as returned by
	 * {@link #escape(Appendable)}) which can cope with arbitrarily split input.
	 * 
	 * <p>
	 * <b>Note:</b> When implementing an escaper it is a good idea to override this method for efficiency by inlining the implementation of {@link #nextEscapeIndex(CharSequence, int, int)} directly. Doing this for {@link PercentEscaper}
	 * more than doubled the performance for unescaped strings (as measured by {@link CharEscapersBenchmark}).
	 * 
	 * @param string
	...
	/**
	 * Returns the escaped form of a given literal string, starting at the given index. This method is called by the {@link #escape(String)} method when it discovers that escaping is required. It is protected to allow subclasses to override
	 * the fastpath escaping function to inline their escaping test. See {@link CharEscaperBuilder} for an example usage.
	 * 
	 * <p>
	...
	/**
	 * Returns an {@code Appendable} instance which automatically escapes all text appended to it before passing the resulting text to an underlying {@code Appendable}.
	 * 
	 * <p>
	 * Unlike {@link #escape(String)} it is permitted to append arbitrarily split input to this Appendable, including input that is split over a surrogate pair. In this case the pending high surrogate character will not be processed until
	 * the corresponding low surrogate is appended. This means that a trailing high surrogate character at the end of the input cannot be detected and will be silently ignored. This is unavoidable since the Appendable interface has no
	 * {@code close()} method, and it is impossible to determine when the last characters have been appended.
	 * 
	 * <p>
	 * The methods of the returned object will propagate any exceptions thrown by the underlying {@code Appendable}.
	 * 
	 * <p>
	 * For well formed <a href="http://en.wikipedia.org/wiki/UTF-16">UTF-16</a> the escaping behavior is identical to that of {@link #escape(String)} and the following code is equivalent to (but much slower than)
		...
		return new Appendable() {
			...
			public Appendable append(char c) throws IOException {
				if (pendingHighSurrogate != -1) {
					...
					// surrogate pair
					// so we have to do some extra work first.
					if (!Character.isLowSurrogate(c)) { throw new IllegalArgumentException("Expected low surrogate character but got '" + c + "' with value " + (int) c); }
					char[] escaped = escape(Character.toCodePoint((char) pendingHighSurrogate, c));
					if (escaped != null) {
						outputChars(escaped, escaped.length);
					} else {
					...
					if (Character.isLowSurrogate(c)) { throw new IllegalArgumentException("Unexpected low surrogate character '" + c + "' with value " + (int) c); }
	 ...
	 * <li>If the first character was a high surrogate value, then an attempt is made to read the next character.
	 * <ol>
	 * <li><b>If the end of the sequence was reached, the negated value of the trailing high surrogate is returned.</b>
	 * <li><b>If the next character was a valid low surrogate, the code point value of the high/low surrogate pair is returned.</b>
	 * <li>If the next character was not a low surrogate value, then {@link IllegalArgumentException} is thrown.
	 * </ol>
	 * <li>If the first character was a low surrogate value, {@link IllegalArgumentException} is thrown.
	 * </ol>
	 * 
	 * @param seq
