...
/**
 * Convenience utility to parse JavaBeans. When the YAML document contains a global tag with the class definition like '!!com.package.MyBean' it is ignored in favour of the runtime class <code>T</code>.
 * 
...
public class JavaBeanLoader<T> {
	...
	public JavaBeanLoader(LoaderOptions options, BeanAccess beanAccess) {
		if (options == null) { throw new NullPointerException("LoaderOptions must be provided."); }
		if (options.getRootTypeDescription() == null) { throw new NullPointerException("TypeDescription must be provided."); }
