...
public class Yaml {
	 ...
	 * 
	 * <p>
	 * This method is similar to <code>Yaml.dump(data)</code> except that the root tag for the whole document is replaced with the given tag. This has two main uses.
	 * </p>
	 * 
	 * <p>
	 * First, if the root tag is replaced with a standard YAML tag, such as <code>Tag.MAP</code>, then the object will be dumped as a map. The root tag will appear as <code>!!map</code>, or blank (implicit !!map).
	 * </p>
	 * 
	 * <p>
	 * Second, if the root tag is replaced by a different custom tag, then the document appears to be a different type when loaded. For example, if an instance of MyClass is dumped with the tag !!YourClass, then it will be handled as an
	 * instance of YourClass when loaded.
	 * </p>
	 * 
	 * @param data
	 *            Java object to be serialized to YAML
	 * @param rootTag
	 *            the tag for the whole YAML document. The tag should be Tag.MAP for a JavaBean to make the tag disappear (to use implicit tag !!map). If <code>null</code> is provided then the standard tag with the full class name is used.
	 * @param flowStyle
	 *            flow style for the whole document. See Chapter 10. Collection Styles http://yaml.org/spec/1.1/#id930798. If <code>null</code> is provided then the flow style from DumperOptions is used.
	 * 
	 * @return YAML String
	 */
	 ...
	 * </p>
	 * <p>
	 * This method is similar to <code>Yaml.dump(data)</code> except that the root tag for the whole document is replaced with <code>Tag.MAP</code> tag (implicit !!map).
	 * </p>
	 * <p>
