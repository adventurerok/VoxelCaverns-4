...
public class PlantGenReeds implements PlantGenerator {
	...
	public void generate(long x, long y, long z, Plant plant) {
		...
		if (world.getBlockId(x + 1, y - 1, z) != Vanilla.water.uid && world.getBlockId(x - 1, y - 1, z) != Vanilla.water.uid && world.getBlockId(x, y - 1, z + 1) != Vanilla.water.uid
				&& world.getBlockId(x, y - 1, z - 1) != Vanilla.water.uid) { return; }
		for (int d = 0; d < max; ++d) {
