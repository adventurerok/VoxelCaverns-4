...
public class BlockAndGate extends BlockLogicGate implements IBlockWrenchable {

	int side1[] = new int[15];
	int side2[] = new int[15];
	int lookup[][] = new int[6][6];

	public BlockAndGate(int uid) {
		super(uid);
		int dat = 0;
		for (int a = 0; a < 6; ++a) {
			for (int b = a + 1; b < 6; ++b) {
	...
	public int getTextureIndexGate(World world, long x, long y, long z, int side) {
		...
		if (side1[data] == side) return BlockTexture.andGateInputA;
		if (side2[data] == side) return BlockTexture.andGateInputB;
		return BlockTexture.andGateOutput;
	}
