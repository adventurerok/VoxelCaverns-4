...
public class BlockReeds extends Block {
		...
		if (item.getDamage() == 0) {
			for (int d = 1; d < 3; ++d) {
				if (world.getBlockId(x, y - d, z) != uid) {
					if (world.getBlockId(x + 1, y - d, z) != Vanilla.water.uid && world.getBlockId(x - 1, y - d, z) != Vanilla.water.uid && world.getBlockId(x, y - d, z + 1) != Vanilla.water.uid
							&& world.getBlockId(x, y - d, z - 1) != Vanilla.water.uid) { return; }
				}
			}
		}
	...
	public void nearbyBlockChanged(World world, long x, long y, long z, Direction dir) {
		...
		if (data == 0) {
			for (int d = 1; d < 3; ++d) {
				if (world.getBlockId(x, y - d, z) != uid) {
					if (world.getBlockId(x + 1, y - d, z) != Vanilla.water.uid && world.getBlockId(x - 1, y - d, z) != Vanilla.water.uid && world.getBlockId(x, y - d, z + 1) != Vanilla.water.uid
							&& world.getBlockId(x, y - d, z - 1) != Vanilla.water.uid) {
						for (int a = 0; a < d; ++a) {
	...
	public int blockUpdate(World world, Random rand, long x, long y, long z, byte data, int buid) {
		for (int d = 1; d < 7; ++d) {
			if (world.getBlockId(x, y - d, z) != uid) {
				if (data == 0 && world.getBlockId(x + 1, y - d, z) != Vanilla.water.uid && world.getBlockId(x - 1, y - d, z) != Vanilla.water.uid && world.getBlockId(x, y - d, z + 1) != Vanilla.water.uid
						&& world.getBlockId(x, y - d, z - 1) != Vanilla.water.uid) {
					for (int a = 0; a < d; ++a) {
						world.setBlockId(x, y - a, z, 0);
					}
