...
public class BlockRendererChair extends BlockRendererDefault {
	...
	AABB[] legs = new AABB[] { AABB.getBoundingBox(min, min + 1 / 16d, 0, legMax, min, min + 1 / 16d), AABB.getBoundingBox(15 / 16d - min, max, 0, legMax, min, min + 1 / 16d),
			AABB.getBoundingBox(15 / 16d - min, max, 0, legMax, 15 / 16d - min, max), AABB.getBoundingBox(min, min + 1 / 16d, 0, legMax, 15 / 16d - min, max) };
	AABB[] sides = new AABB[] { AABB.getBoundingBox(max - small, max, baseMax, 1, min, max), AABB.getBoundingBox(min, max, baseMax, 1, max - small, max), AABB.getBoundingBox(min, min + small, baseMax, 1, min, max),
			AABB.getBoundingBox(min, max, baseMax, 1, min, min + small) };
	...
	public void renderBlock(Chunk c, MapData m, int cx, int cy, int cz, Block block, byte data, Renderer[] renderers) {
		...
		if ((data & 1) != 0) {
			for (int d = 0; d < 5; ++d)
				renderBlockFace(c.getWorld(), x, y, z, block, renderers[block.getRendererToUse(data, d)], sides[0], d);
		}
		if ((data & 2) != 0) {
			for (int d = 0; d < 5; ++d)
				renderBlockFace(c.getWorld(), x, y, z, block, renderers[block.getRendererToUse(data, d)], sides[1], d);
		}
		if ((data & 4) != 0) {
			for (int d = 0; d < 5; ++d)
				renderBlockFace(c.getWorld(), x, y, z, block, renderers[block.getRendererToUse(data, d)], sides[2], d);
		}
		if ((data & 8) != 0) {
			for (int d = 0; d < 5; ++d)
				renderBlockFace(c.getWorld(), x, y, z, block, renderers[block.getRendererToUse(data, d)], sides[3], d);
		}
		...
		if ((data & 1) != 0) {
			for (int d = 0; d < 5; ++d)
				renderBlockFace(c.getWorld(), x, y, z, mt, renderers[block.getRendererToUse(data, d)], sides[0], d);
		}
		if ((data & 2) != 0) {
			for (int d = 0; d < 5; ++d)
				renderBlockFace(c.getWorld(), x, y, z, mt, renderers[block.getRendererToUse(data, d)], sides[1], d);
		}
		if ((data & 4) != 0) {
			for (int d = 0; d < 5; ++d)
				renderBlockFace(c.getWorld(), x, y, z, mt, renderers[block.getRendererToUse(data, d)], sides[2], d);
		}
		if ((data & 8) != 0) {
			for (int d = 0; d < 5; ++d)
				renderBlockFace(c.getWorld(), x, y, z, mt, renderers[block.getRendererToUse(data, d)], sides[3], d);
		}
	}
	...
	public void renderBlockCracks(World world, long x, long y, long z, Renderer render, double amount) {
		...
		if ((data & 1) != 0) {
			for (int d = 0; d < 5; ++d)
				renderBlockFaceCracks(world, x, y, z, block, render, sides[0], d, amount);
		}
		if ((data & 2) != 0) {
			for (int d = 0; d < 5; ++d)
				renderBlockFaceCracks(world, x, y, z, block, render, sides[1], d, amount);
		}
		if ((data & 4) != 0) {
			for (int d = 0; d < 5; ++d)
				renderBlockFaceCracks(world, x, y, z, block, render, sides[2], d, amount);
		}
		if ((data & 8) != 0) {
			for (int d = 0; d < 5; ++d)
				renderBlockFaceCracks(world, x, y, z, block, render, sides[3], d, amount);
		}
	}
}
