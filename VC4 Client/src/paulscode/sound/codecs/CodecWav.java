...
public class CodecWav implements ICodec {
	...
	public SoundBuffer read() {
		...
		try {
			...
			while ((!endOfStream(GET, YYY)) && (bytesRead < streamBuffer.length)) {
			...
			// : See if setting endOfStream is needed here
			endOfStream(SET, true);
			return null;
		}
	...
	public SoundBuffer readAll() {
		...
		if (fileSize > 0) {
			...
			while ((!endOfStream(GET, YYY)) && (totalBytes < SoundSystemConfig.getMaxFileSize())) {
				bytesRead = 0;
				cnt = 0;
