...
/**
 * The Library class is the class from which all library types are extended. It provides generic methods for interfacing with the audio libraries supported by the SoundSystem. Specific libraries should extend this class and override the
 * necessary methods. For consistant naming conventions, each sub-class should have the name prefix "Library".
 * 
 * This class may also be used as the "No Sound Library" (i.e. silent mode) if no other audio libraries are supported by the host machine, or to mute all sound. <br>
 * <br>
...
public class Library {
	...
	/**
	 * Constructor: Instantiates the source map and listener information. NOTES: The 'super()' method should be at the top of constructors for all extended classes. The varriable 'libraryType' should be given a new value in the constructors
	 * for all extended classes.
	 */
	...
	/**
	 * Removes a pre-loaded sound from memory. This is a good method to use for freeing up memory after a large sound file is no longer needed. NOTE: the source will remain in memory after this method has been called, for as long as the
	 * sound is attached to an existing source.
	 * 
	 * @param filename
	...
	/**
	 * Returns a handle to the next available channel. If the specified source is a normal source, a normal channel is returned, and if it is a streaming source, then a streaming channel is returned. If all channels of the required type are
	 * currently playing, then the next channel playing a non-priority source is returned. If no channels are available (i.e. they are all playing priority sources) then getNextChannel returns null.
	 * 
	 * @param source
	 ...
	 * sound finished playing. This method may only be used for streaming and MIDI sources.
	 * 
	 * @param sourcename
	 ...
	 * played next when the current sound finished playing. This method may only be used for streaming and MIDI sources.
	 * 
	 * @param sourcename
