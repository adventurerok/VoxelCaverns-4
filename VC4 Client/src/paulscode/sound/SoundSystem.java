...
/**
 * The SoundSystem class is the core class for the SoundSystem library. It is capable of interfacing with external sound library and codec library pluggins. This core class is stripped down to give it a smaller memory footprint and to make
 * it more customizable. This library was created to provide a simple, common interface to a variety of 3rd-party sound and codec libraries, and to simplify switching between them on the fly. If no external pluggins are loaded, this core
 * class by itself is only capable of playing MIDI files. Specific implementations (such as SoundSystemJPCT) will extend this core class. They will automatically link with popular external pluggins and provide extra methods for ease of use.
 * There should be only one instance of this class in any program! The SoundSystem can be constructed by defining which sound library to use, or by allowing SoundSystem to perform its own library compatibility checking. See
 * {@link paulscode.sound.SoundSystemConfig SoundSystemConfig} for information about changing default settings and linking with external pluggins. <br>
 * <br>
...
public class SoundSystem {
	...
	/**
	 * Constructor: Create the sound system using the default library. If the default library is not compatible, another library type will be loaded instead, in the order of library preference. See {@link paulscode.sound.SoundSystemConfig
	 * SoundSystemConfig} for information about sound library types.
	 */
	public SoundSystem() {
	 ...
	 * 
	 * @param libraryClass
	 *            Library to use. See {@link paulscode.sound.SoundSystemConfig SoundSystemConfig} for information about chosing a sound library.
	 */
	...
	/**
	 * Links with any default libraries or codecs should be made in this method. This method is empty in the core SoundSystem class, and should be overriden by classes which extend SoundSystem. See {@link paulscode.sound.SoundSystemConfig
	 * SoundSystemConfig} for information about linking with sound libraries and codecs.
	 */
	 ...
	 * 
	 * @param libraryClass
	 *            Library to initialize. See {@link paulscode.sound.SoundSystemConfig SoundSystemConfig} for information about chosing a sound library.
	 */
	...
	/**
	 * Wakes up the Command Thread to process commands. This method should be used if there is a need to call methods 'ManageSources' and 'CommandQueue' externally. In most cases, this method will not be needed, since SoundSystem
	 * automatically wakes the Command Thread every time commands are placed into either the ManageSources queue or CommandQueue to be processed.
	 */
	...
	/**
	 * Pre-loads a sound into memory. The file may either be located within the JAR or at an online location. If the file is online, filename must begin with "http://", since that is how SoundSystem recognizes URL's. If the file is located
	 * within the compiled JAR, the package in which sound files are located may be set by calling SoundSystemConfig.setSoundFilesPackage().
	 * 
	 * @param filename
	...
	/**
	 * Pre-loads a sound specified by the given URL into memory. The second parameter 'identifier' should look like a filename, and it must have the correct extension so SoundSystem knows what codec to use for the file referenced by the URL
	 * instance.
	 * 
	 * @param url
	...
	/**
	 * Removes a pre-loaded sound from memory. This is a good method to use for freeing up memory after a large sound file is no longer needed. NOTE: the source will remain in memory after calling this method as long as the sound is
	 * attached to an existing source. When calling this method, calls should also be made to method removeSource( String ) for all sources which this sound is bound to.
	 * 
	 * @param filename
	...
	/**
	 * If the specified source is a streaming source or MIDI source, this method queues up the next sound to play when the previous playback ends. The file may either be located within the JAR or at an online location. If the file is
	 * online, filename must begin with "http://", since that is how SoundSystem recognizes URL paths. If the file is located within the compiled JAR, the package in which sound files are located may be set by calling
	 * SoundSystemConfig.setSoundFilesPackage(). This method has no effect on non-streaming sources.
	 * 
	 * @param sourcename
	 *            Source identifier.
	 * @param filename
	...
	/**
	 * If the specified source is a streaming source or MIDI source, this method queues up the next sound to play when the previous playback ends. The third parameter 'identifier' should look like a filename, and it must have the correct
	 * extension so SoundSystem knows what codec to use for the file referenced by the URL instance. This method has no effect on non-streaming sources.
	 * 
	 * @param sourcename
	 *            Source identifier.
	 * @param url
	...
	/**
	 * Fades out the volume of whatever the specified source is currently playing, then begins playing the specified file at the source's previously assigned volume level. The file may either be located within the JAR or at an online
	 * location. If the file is online, filename must begin with "http://", since that is how SoundSystem recognizes URL paths. If the file is located within the compiled JAR, the package in which sound files are located may be set by
	 * calling SoundSystemConfig.setSoundFilesPackage(). If the filename parameter is null or empty, the specified source will simply fade out and stop. The miliseconds parameter must be non-negative or zero. This method will remove
	 * anything that is currently in the specified source's list of queued sounds that would have played next when the current sound finished playing. This method may only be used for streaming and MIDI sources.
	 * 
	 * @param sourcename
	...
	/**
	 * Fades out the volume of whatever the specified source is currently playing, then begins playing the specified file at the source's previously assigned volume level. If the url parameter is null or empty, the specified source will
	 * simply fade out and stop. The third parameter 'identifier' should look like a filename, and it must have the correct extension so SoundSystem knows what codec to use for the file referenced by the URL instance. The miliseconds
	 * parameter must be non-negative or zero. This method will remove anything that is currently in the specified source's list of queued sounds that would have played next when the current sound finished playing. This method may only be
	 ...
	 * level. The filename parameter may not be null or empty. The file may either be located within the JAR or at an online location. If the file is online, filename must begin with "http://", since that is how SoundSystem recognizes URL
	 * paths. If the file is located within the compiled JAR, the package in which sound files are located may be set by calling SoundSystemConfig.setSoundFilesPackage(). The miliseconds parameters must be non-negative or zero. This method
	 * will remove anything that is currently in the specified source's list of queued sounds that would have played next when the current sound finished playing. This method may only be used for streaming and MIDI sources.
	 * 
	 * @param sourcename
	 ...
	 * level. The url parameter may not be null or empty. The third parameter 'identifier' should look like a filename, and it must have the correct extension so SoundSystem knows what codec to use for the file referenced by the URL
	 * instance. The miliseconds parameters must be non-negative or zero. This method will remove anything that is currently in the specified source's list of queued sounds that would have played next when the current sound finished
	...
	/**
	 * Makes sure the current volume levels of streaming sources and MIDI are correct. This method is designed to help reduce the "jerky" fading behavior that happens when using some library and codec pluggins (such as LibraryJavaSound and
	 * CodecJOrbis). This method has no effect on normal "non-streaming" sources. It would normally be called somewhere in the main "game loop". IMPORTANT: To optimize frame-rates, do not call this method for every frame. It is better to
	 * just call this method at some acceptable "granularity" (play around with different granularities to find what sounds acceptable for a particular situation).
	 */
	...
	/**
	 * Creates a new permanant, streaming, priority source with zero attenuation. The file may either be located within the JAR or at an online location. If the file is online, filename must begin with "http://", since that is how
	...
	/**
	 * Creates a new permanant, streaming, priority source with zero attenuation. The third parameter 'identifier' should look like a filename, and it must have the correct extension so SoundSystem knows what codec to use for the file
	...
	/**
	 * Creates a new streaming source. The file may either be located within the JAR or at an online location. If the file is online, filename must begin with "http://", since that is how SoundSystem recognizes URL paths. If the file is
	 * located within the compiled JAR, the package in which sound files are located may be set by calling SoundSystemConfig.setSoundFilesPackage().
	 * 
	 * @param priority
	 *            Setting this to true will prevent other sounds from overriding this one.
	 * @param sourcename
	...
	/**
	 * Opens a direct line for streaming audio data. This method creates a new streaming source to play the data at. The resulting streaming source can be manipulated the same as any other streaming source. Raw data can be sent to the new
	 * streaming source using the feedRawAudioData() method.
	 * 
	 * @param audioFormat
	...
	/**
	 * Creates a temporary source and plays it. After the source finishes playing, it is removed. Returns a randomly generated name for the new source. NOTE: to make a source created by this method permanant, call the setActive() method
	...
	/**
	 * Creates a temporary source and streams it. After the source finishes playing, it is removed. The file may either be located within the JAR or at an online location. If the file is online, filename must begin with "http://", since
	 * that is how SoundSystem recognizes URL paths. If the file is located within the compiled JAR, the package in which sound files are located may be set by calling SoundSystemConfig.setSoundFilesPackage(). Returns a randomly generated
	...
	/**
	 * Creates a temporary source and streams it. After the source finishes playing, it is removed. The third parameter 'identifier' should look like a filename, and it must have the correct extension so SoundSystem knows what codec to use
	 * for the file referenced by the URL instance. Returns a randomly generated name for the new source. NOTE: to make a source created by this method permanant, call the setActive() method using the return value for sourcename.
	 * 
	 * @param priority
	 *            Setting this to true will prevent other sounds from overriding this one.
	 * @param url
	 ...
	 *            Identifier for the source.
	 * @param dr
	 *            Either the fading distance or rolloff factor, depending on the attenuation model used.
	 */
	...
	/**
	 * Feeds raw data through the specified source. The source must be a streaming source and it can not be already associated with a file or URL to stream from. Only use this for streaming sources created with the rawDataStream() method.
	 * NOTE: Be carefull how much data you send to a source to stream. The data will be processed at playback speed, so if you queue up 1 hour worth of data, it will take 1 hour to play (not to mention hogging a ton of memory). To clear out
	 * all queued data from the source, use the flush() method. Also note: if there is a break in the data stream, you will hear clicks and studders, so ensure that the data flow is steady.
	 * 
	 * @param sourcename
	...
	/**
	 * Sets a flag for a source indicating whether it should be used or if it should be removed after it finishes playing. One possible use for this method is to make temporary sources that were created with quickPlay() permanant. Another
	 * use could be to have a source automatically removed after it finishes playing. NOTE: Setting a source to temporary does not stop it, and setting a source to permanant does not play it. It is also important to note that a looping
	 * temporary source will not be removed as long as it keeps playing.
	 * 
	 * @param sourcename
	 *            Identifier for the source.
	 * @param temporary
	...
	public boolean newLibrary(Class<?> libraryClass) throws SoundSystemException {
		...
		for (int x = 0; (!initialized(GET, YYY)) && (x < 100); x++) {
			snooze(400);
			commandThread.interrupt();
		}

		if (!initialized(GET, YYY)) {
	...
	/**
	 * Saves the specified sample data, under the specified identifier. This identifier can be later used in place of 'filename' parameters to reference the sample data. This method is used internally by SoundSystem for thread
	 * synchronization, and it can not be called directly - please use the loadSound() method instead.
	 * 
	 * @param buffer
	...
	/**
	 * If the specified source is a streaming source or MIDI source, this method queues up the next sound to play when the previous playback ends. This method has no effect on non-streaming sources. This method is used internally by
	 * SoundSystem for thread synchronization, and it can not be called directly - please use the queueSound() method instead.
	 * 
	 * @param sourcename
	 *            Source identifier.
	 * @param filenameURL
	...
	/**
	 * Removes the first occurrence of the specified filename/identifier from the specified source's list of sounds to play when previous playback ends. This method has no effect on non-streaming sources. This method is used internally by
	 * SoundSystem for thread synchronization, and it can not be called directly - please use the dequeueSound() method instead.
	 * 
	 * @param sourcename
	 *            Source identifier.
	 * @param filename
	 *            Filename/identifier of the sound file to remove from the queue.
	 */
	...
	/**
	 * Fades out the volume of whatever the specified source is currently playing, then begins playing the specified file at the source's previously assigned volume level. If the filenameURL parameter is null or empty, the specified source
	 * will simply fade out and stop. The miliseconds parameter must be non-negative or zero. This method will remove anything that is currently in the specified source's list of queued sounds that would have played next when the current
	 * sound finished playing. This method may only be used for streaming and MIDI sources. This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please use the fadeOut() method
	 * instead.
	 * 
	 * @param sourcename
	 *            Name of the source to fade out.
	...
	/**
	 * Fades out the volume of whatever the specified source is currently playing, then fades the volume back in playing the specified file. Final volume after fade-in completes will be equal to the source's previously assigned volume
	 * level. The filenameURL parameter may not be null or empty. The miliseconds parameters must be non-negative or zero. This method will remove anything that is currently in the specified source's list of queued sounds that would have
	 * played next when the current sound finished playing. This method may only be used for streaming and MIDI sources. This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please
	 * use the fadeOutIn() method instead.
	 * 
	 * @param sourcename
	 *            Name of the source to fade out/in.
	 * @param filenameURL
	 ...
	 * Makes sure the current volume levels of streaming sources and MIDI are correct. This method is designed to help reduce the "jerky" fading behavior that happens when using some library and codec pluggins (such as LibraryJavaSound and
	 * CodecJOrbis). This method has no effect on normal "non-streaming" sources. It would normally be called somewhere in the main "game loop". IMPORTANT: To optimize frame-rates, do not call this method for every frame. It is better to
	 * just call this method at some acceptable "granularity" (play around with different granularities to find what sounds acceptable for a particular situation). This method is used internally by SoundSystem for thread synchronization,
	 * and it can not be called directly - please use the checkFadeVolumes() method instead.
	 */
	...
	/**
	 * Creates a temporary source and either plays or streams it. After the source finishes playing, it is removed. This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please use the
	 * quickPlay() method instead.
	 * 
	 * @param priority
	 *            Setting this to true will prevent other sounds from overriding this one.
	 * @param toStream
	 ...
	 *            Either the fading distance or rolloff factor, depending on the value of "attmodel".
	 * @param temporary
	 *            Whether or not the source should be removed after it finishes playing.
	 */
	...
	/**
	 * Set a source's priority factor. A priority source will not be overriden when too many sources are playing at once. This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please
	 * use the setPriority() method instead.
	 * 
	 * @param sourcename
	 *            Identifier for the source.
	 * @param pri
	...
	/**
	 * Changes a source's attenuation model. This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please use the setAttenuation() method instead. See
	 * {@link paulscode.sound.SoundSystemConfig SoundSystemConfig} for more information about Attenuation.
	 * 
	 * @param sourcename
	 *            Identifier for the source.
	 * @param model
	...
	/**
	 * Changes the Doppler factor. This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please use the setDopplerFactor() method instead. See {@link paulscode.sound.SoundSystemConfig
	 * SoundSystemConfig} for more information about Doppler effect.
	 * 
	 * @param dopplerFactor
	...
	/**
	 * Changes the Doppler velocity. This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please use the setDopplerVelocity() method instead. See
	...
	/**
	 * Changes a source's velocity, for use in Doppler effect. This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please use the setVelocity() method instead. See
	 * {@link paulscode.sound.SoundSystemConfig SoundSystemConfig} for more information about Doppler effect.
	 * 
	 * @param sourcename
	 *            Identifier for the source.
	...
	/**
	 * Changes the listener's velocity, for use in Doppler effect. This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please use the setListenerVelocity() method instead. See
	 * {@link paulscode.sound.SoundSystemConfig SoundSystemConfig} for more information about Doppler effect.
	 * 
	 * @param x
	...
	/**
	 * Feeds raw data through the specified source. The source must be a streaming source and it can not be already associated with a file or URL to stream from. This method is used internally by SoundSystem for thread synchronization, and
	 * it can not be called directly - please use the feedRawAudioData() method instead.
	 * 
	 * @param sourcename
	 *            Name of the streaming source to play from.
	 * @param buffer
	...
	/**
	 * Sets a flag for a source indicating whether it should be used or if it should be removed after it finishes playing. One possible use for this method is to make temporary sources that were created with quickPlay() permanant. Another
	 * use could be to have a source automatically removed after it finishes playing. NOTE: Setting a source to inactive does not stop it, and setting a source to active does not play it. It is also important to note that a looping inactive
	 * source will not be removed as long as it keeps playing. This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please use the setTemporary() method instead.
	 * 
	 * @param sourcename
	 *            Identifier for the source.
	 * @param temporary
	...
	/**
	 * Turns the listener counterclockwise by "angle" radians around the y-axis, relative to the current angle. This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please use the
	 * turnListener() method instead.
	 * 
	 * @param angle
	 *            radian offset.
	 */
	...
	/**
	 * Culls the specified source. A culled source can not be played until it has been activated again. This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please use the cull()
	 * method instead.
	 * 
	 * @param sourcename
	 *            Identifier for the source.
	...
	/**
	 * This method can be overridden by extended classes to be used for source management (culling and activating sources based on established rules). One possible use for this method is sorting sources by distance, culling the furthest,
	 * and activating the closest. This method is automatically called on the CommandThread before processing queued commands, so you do not need to call it anywhere else. Note: use methods cull() and activate() here, NOT CommandCull() and
	 * CommandActivate() (thread safety issue). IMPORTANT: Always use synchronized( SoundSystemConfig.THREAD_SYNC ) when manually manipulating sources from outside the Command Thread!
	 */
	...
	/**
	 * Queues a command. If newCommand is null, all commands are dequeued and executed. This is automatically used by the sound system, so it is not likely that a user would ever need to use this method. See
	 * {@link paulscode.sound.CommandObject CommandObject} for more information about commands.
	 * 
	 * @param newCommand
	 ...
	 */
	private static HashMap<String, Source> copySources(HashMap<String, Source> sourceMap) {
		Set<String> keys = sourceMap.keySet();
		Iterator<String> iter = keys.iterator();
		String sourcename;
		Source source;
	 ...
	 * @param value
	 *            New value if action is SET, otherwise YYY.
	 ...
	 *            Action to perform (GET or SET).
	 * @param e
	 *            New exception if action is SET, otherwise YYY.
