...
/**
 * The SoundSystemConfig class is used to access global sound system settings, and to link with external pluggins. All members of this class are static. SoundSystemConfig is sort of a "catch all" configuration class, so if you are not sure
 * where to find something in the SoundSystem library, this is probably a good place to start. <br>
 * <br>
...
public class SoundSystemConfig {
	...
	/**
	 * Global identifier for rolloff attenuation. Rolloff attenuation is a realistic attenuation model, which uses a rolloff factor to determine how quickly a source fades with distance. A smaller rolloff factor will fade at a further
	 * distance, and a rolloff factor of 0 will never fade. NOTE: In OpenAL, rolloff attenuation only works for monotone sounds.
	 */
	...
	/**
	 * Handle to the message logger. The default logger can be changed by overridding the {@link paulscode.sound.SoundSystemLogger SoundSystemLogger} class and calling the setLogger() method (must be done BEFORE instantiating the
	 * SoundSystem class!)
	 */
	...
	/**
	 * Enables a transition-speed optimization by assuming all sounds in each streaming source's queue will have exactly the same format once decoded (including channels, sample rate, and sample size). This is an advanced setting which
	...
	/**
	 * MIDI device to try using as the Synthesizer. May be the full name or part of the name. If this String is empty, the default Synthesizer will be used, or one of the common alternate synthesizers if the default Synthesizer is
	 * unavailable.
	 */
	...
	/**
	 * Adds an entry to the list of library types. This method has no effect if the specified library type is already in the list of libraries. NOTE: The parameterless constructor of the SoundSystem class will try to load libraries in the
	 * order that they were entered into the list.
	 * 
	 * @param libraryClass
	...
	/**
	 * Changes the message logger to use for handling status messages, warnings, and error messages. This method should only be called BEFORE instantiating the SoundSystem class! If this method is called after the SoundSystem has been
	 * created, there will be handles floating around to two different loggers, and the results will be undesirable. This method can be used to change how messages are handled. First, the {@link paulscode.sound.SoundSystemLogger
	 * SoundSystemLogger} class should be extended and methods overriden to change how messages are handled. Then, the overridden class should be instantiated, and a call made to SoundSystemConfig.setLogger() before creating the SoundSystem
	 * object. If an alternate logger is not set by the user before the SoundSystem is instantiated, then an instance of the base SoundSystemLogger class will be used by default.
	 * 
	 * @param l
	...
	/**
	 * Sets the maximum number of normal (non-streaming) channels that can be created. Streaming channels are created first, so the higher the maximum number of streaming channels is set, the fewer non-streaming channels will be available.
	 * If unable to create the number of channels specified, SoundSystem will create as many as possible. NOTE: Some sound library pluggins may require the total number of channels (non-streaming + streaming) to be 32.
	 * 
	 * @param number
	...
	/**
	 * Sets the maximum number of streaming channels that can be created. Streaming channels are created first, so the higher the maximum number of streaming channels is set, the fewer non-streaming channels will be available. If unable to
	 * create the number of channels specified, SoundSystem will create as many as possible. NOTE: Some sound library pluggins may require the total number of channels (non-streaming + streaming) to be 32.
	 * 
	 * @param number
	...
	/**
	 * Sets the doppler factor, for determining Doppler Effect scale. Use this method BEFORE instantiating the SoundSystem. To change the Doppler factor after the SoundSystem is instantiated, use the SoundSystem.changeDopplerFactor method
	 * instead.
	 * 
	 * @param factor
	 *            Doppler factor.
	...
	/**
	 * Sets the Doppler velocity, for use in Doppler Effect. Use this method BEFORE instantiating the SoundSystem. To change the Doppler velocity after the SoundSystem is instantiated, use the SoundSystem.changeDopplerVelocity method
	 * instead.
	 * 
	 * @param velocity
	 *            Doppler velocity.
	...
	/**
	 * Returns whether or not all sounds in each streaming source's queue will be handled as if they have exactly the same format once decoded (including channels, sample rate, and sample size). This is an advanced setting which should only
	 * be changed by experienced developers.
	 * 
	 ...
	 * @param queueSize
	 *            Number of items left the the stream's play queue, or zero if none.
	 */
	...
	private static class Codec {
		...
		private static void instantiationErrorMessage() {
