...
/**
 * The Source class is used to store information about a source. Source objects are stored in a map in the Library class. The information they contain is used to create library-specific sources. This is the template class which is extended
 * for each specific library. This class is also used by the "No Sound" library to represent a mute source. <br>
 * <br>
...
public class Source {
	...
	/**
	 * Fades out the volume of whatever this source is currently playing, then begins playing the specified filename at the source's previously assigned volume level. If the filename parameter is null or empty, the source will simply fade
	 * out and stop. The miliseconds parameter must be non-negative or zero. This method will remove anything that is currently in the list of queued sounds that would have played next when the current sound finished playing. This method
	...
	/**
	 * Fades out the volume of whatever this source is currently playing, then fades the volume back in playing the specified file. Final volume after fade-in completes will be equal to the source's previously assigned volume level. The
	 * filenameURL parameter may not be null or empty. The miliseconds parameters must be non-negative or zero. This method will remove anything that is currently in the list of queued sounds that would have played next when the current
	 * sound finished playing. This method has no effect on non-streaming sources.
	 * 
	 * @param filenameURL
	 *            Filename/URL of the sound file to play next, or null for none.
	 * @param milisOut
	...
	/**
	 * Resets this source's volume if it is fading out or in. Returns true if this source is currently in the process of fading out. When fade-out completes, this method transitions the source to the next sound in the sound sequence queue
	 * if there is one. This method has no effect on non-streaming sources.
	 * 
	...
	/**
	 * Removes the next filename/URL from the sound sequence queue and assigns it to this source. This method has no effect on non-streaming sources. This method is used internally by SoundSystem, and it is unlikely that the user will ever
	 * need to use it.
	 * 
	...
	/**
	 * Reads in initial buffers of data from the next sound in the sound sequence queue, to reduce lag when the transition occurrs. This method has no effect on non-streaming sources. This method is used internally by SoundSystem, and it is
	...
	public boolean stream() {
		...
		if (rawDataStream) {
			...
			for (int i = 0; i < processed; i++) {
				...
				/*
				 * if( codec.endOfStream() ) { synchronized( soundSequenceLock ) { if( SoundSystemConfig.getStreamQueueFormatsMatch() ) { if( soundSequenceQueue != null && soundSequenceQueue.size() > 0 ) { if( codec != null )
				 * codec.cleanup(); filenameURL = soundSequenceQueue.remove( 0 ); codec = SoundSystemConfig.getCodec( filenameURL.getFilename() ); codec.initialize( filenameURL.getURL() ); return true; } else if( toLoop ) {
				 * codec.initialize( filenameURL.getURL() ); buffer = codec.read(); if( buffer != null ) { if( buffer.audioData != null ) channel.queueBuffer( buffer.audioData ); buffer.cleanup(); buffer = null; } } } } return false; }
				 */
			}
		}
		return false;
