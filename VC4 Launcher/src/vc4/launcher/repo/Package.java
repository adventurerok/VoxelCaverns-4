...
public class Package {
	...
	private Runnable[] runs = new Runnable[0];
	...
	public void load(YamlMap map) {
		...
		for (int d = 0; d < ins.length; ++d)
			installs[d] = new Install(ins[d].toString());
		...
		if (map.hasKey("runnables")) {
			Iterator<YamlMap> runners = map.getSubMap("runnables").getSubMapsIterator();
			ArrayList<Runnable> runList = new ArrayList<>();
			while (runners.hasNext()) {
				YamlMap running = runners.next();
				Runnable ran = new Runnable(running);
				if (running.hasKey("default") && running.getBoolean("default")) runList.add(0, ran);
				else runList.add(ran);
			}
			runs = runList.toArray(new Runnable[runList.size()]);
		}
		try {
			load();
