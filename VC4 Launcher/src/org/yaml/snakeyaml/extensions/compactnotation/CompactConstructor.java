...
public class CompactConstructor extends Constructor {
	...
	protected void setProperties(Object bean, Map<String, Object> data) throws Exception {
		if (data == null) { throw new NullPointerException("Data for Compact Object Notation cannot be null."); }
	...
	public CompactData getCompactData(String scalar) {
		if (!scalar.endsWith(")")) { return null; }
		if (scalar.indexOf('(') < 0) { return null; }
		...
		if (node instanceof MappingNode) {
			...
			if (list.size() == 1) {
				...
				if (key instanceof ScalarNode) {
					...
					if (data != null) { return new ConstructCompactObject(); }
				}
			}
		}
	...
	protected String getSequencePropertyName(Class<?> bean) throws IntrospectionException {
		...
		} else if (properties.size() > 1) { throw new YAMLException("Many list properties found in " + bean + "; Please override getSequencePropertyName() to specify which property to use."); }
