...
public class Constructor extends SafeConstructor {
	...
	public Constructor(TypeDescription theRoot) {
		if (theRoot == null) { throw new NullPointerException("Root type must be provided."); }
	...
	private static final String check(String s) {
		if (s == null) { throw new NullPointerException("Root type must be provided."); }
		if (s.trim().length() == 0) { throw new YAMLException("Root type must be provided."); }
		return s;
	}
	...
	public TypeDescription addTypeDescription(TypeDescription definition) {
		if (definition == null) { throw new NullPointerException("TypeDescription is required."); }
	...
	protected class ConstructMapping implements Construct {
		...
		protected Object createEmptyJavaBean(MappingNode node) {
			try {
				/**
				 * Using only default constructor. Everything else will be initialized on 2nd step. If we do here some partial initialization, how do we then track what need to be done on 2nd step? I think it is better to get only object
				 * here (to have it as reference for recursion) and do all other thing on 2nd step.
				 */
	...
	protected class ConstructScalar extends AbstractConstruct {
		...
		public Object construct(Node nnode) {
			...
			Object result;
			if (type.isPrimitive() || type == String.class || Number.class.isAssignableFrom(type) || type == Boolean.class || Date.class.isAssignableFrom(type) || type == Character.class || type == BigInteger.class
					|| type == BigDecimal.class || Enum.class.isAssignableFrom(type) || Tag.BINARY.equals(node.getTag()) || Calendar.class.isAssignableFrom(type)) {
				...
				if (javaConstructor == null) {
					...
					// it should be possible to use implicit types instead
