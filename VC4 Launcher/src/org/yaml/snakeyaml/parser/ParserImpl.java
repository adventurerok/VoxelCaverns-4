...
public final class ParserImpl implements Parser {
	...
	public boolean checkEvent(Event.ID choices) {
		...
		if (currentEvent != null) {
			if (currentEvent.is(choices)) { return true; }
		}
		return false;
	}
	...
	private class ParseDocumentStart implements Production {
		...
		public Event produce() {
			...
			if (!scanner.checkToken(Token.ID.StreamEnd)) {
				...
				if (!scanner.checkToken(Token.ID.DocumentStart)) { throw new ParserException(null, null, "expected '<document start>', but found " + scanner.peekToken().getTokenId(), scanner.peekToken().getStartMark()); }
				...
				if (!states.isEmpty()) { throw new YAMLException("Unexpected end of stream. States left: " + states); }
				if (!marks.isEmpty()) { throw new YAMLException("Unexpected end of stream. Marks left: " + marks); }
				state = null;
			}
			return event;
		}
	...
	private List<Object> processDirectives() {
		...
		while (scanner.checkToken(Token.ID.Directive)) {
			...
			if (token.getName().equals("YAML")) {
				if (yamlVersion != null) { throw new ParserException(null, null, "found duplicate YAML directive", token.getStartMark()); }
				List<Integer> value = token.getValue();
				Integer major = value.get(0);
				if (major != 1) { throw new ParserException(null, null, "found incompatible YAML document (version 1.* is required)", token.getStartMark()); }
				yamlVersion = token.getValue();
			} else if (token.getName().equals("TAG")) {
				List<String> value = token.getValue();
				String handle = value.get(0);
				String prefix = value.get(1);
				if (tagHandles.containsKey(handle)) { throw new ParserException(null, null, "duplicate tag handle " + handle, token.getStartMark()); }
	...
	private Event parseNode(boolean block, boolean indentlessSequence) {
		...
		if (scanner.checkToken(Token.ID.Alias)) {
			...
			if (tagTokenTag != null) {
				...
				if (handle != null) {
					if (!tagHandles.containsKey(handle)) { throw new ParserException("while parsing a node", startMark, "found undefined tag handle " + handle, tagMark); }
