...
public class Resolver {
	...
	protected void addImplicitResolvers() {
		...
		/*
		 * INT must be before FLOAT because the regular expression for FLOAT matches INT (see issue 130) http://code.google.com/p/snakeyaml/issues/detail?id=130
		 */
		...
		if (kind == NodeId.scalar && implicit) {
			...
			if (resolvers != null) {
				for (ResolverTuple v : resolvers) {
					...
					if (regexp.matcher(value).matches()) { return tag; }
				}
			}
