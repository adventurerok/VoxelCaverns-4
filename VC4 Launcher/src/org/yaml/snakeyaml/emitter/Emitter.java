...
public final class Emitter implements Emitable {
	...
	private class ExpectDocumentStart implements EmitterState {
		...
		public void expect() throws IOException {
			if (event instanceof DocumentStartEvent) {
				...
				boolean implicit = first && !ev.getExplicit() && !canonical && ev.getVersion() == null && ev.getTags() == null && !checkEmptyDocument();
				if (!implicit) {
					writeIndent();
					writeIndicator("---", true, false, false);
					if (canonical) {
	...
	private void expectAlias() throws IOException {
		if (((NodeEvent) event).getAnchor() == null) { throw new EmitterException("anchor is not specified for alias"); }
		processAnchor("*");
		state = states.pop();
	}
	...
	private boolean checkEmptyDocument() {
		if (!(event instanceof DocumentStartEvent) || events.isEmpty()) { return false; }
		Event event = events.peek();
		if (event instanceof ScalarEvent) {
	...
	private Character chooseScalarStyle() {
		...
		if (ev.getStyle() != null && ev.getStyle() == '"' || this.canonical) { return '"'; }
		if (ev.getStyle() == null && ev.getImplicit().canOmitTagInPlainScalar()) {
			if (!(simpleKeyContext && (analysis.empty || analysis.multiline)) && ((flowLevel != 0 && analysis.allowFlowPlain) || (flowLevel == 0 && analysis.allowBlockPlain))) { return null; }
		}
		if (ev.getStyle() != null && (ev.getStyle() == '|' || ev.getStyle() == '>')) {
			if (flowLevel == 0 && !simpleKeyContext && analysis.allowBlock) { return ev.getStyle(); }
		}
	...
	private String prepareVersion(Integer[] version) {
		...
		if (major != 1) { throw new EmitterException("unsupported YAML version: " + version[0] + "." + version[1]); }
	...
	private String prepareTagHandle(String handle) {
		...
		} else if (!"!".equals(handle) && !HANDLE_FORMAT.matcher(handle).matches()) { throw new EmitterException("invalid character in the tag handle: " + handle); }
		return handle;
	}

	private String prepareTagPrefix(String prefix) {
		if (prefix.length() == 0) { throw new EmitterException("tag prefix must not be empty"); }
	...
	private String prepareTag(String tag) {
		if (tag.length() == 0) { throw new EmitterException("tag must not be empty"); }
		if ("!".equals(tag)) { return tag; }
		String handle = null;
		String suffix = tag;
	...
	static String prepareAnchor(String anchor) {
		if (anchor.length() == 0) { throw new EmitterException("anchor must not be empty"); }
		if (!ANCHOR_FORMAT.matcher(anchor).matches()) { throw new EmitterException("invalid character in the anchor: " + anchor); }
		return anchor;
	}

	private ScalarAnalysis analyzeScalar(String scalar) {
		// Empty scalar is a special case.
		if (scalar.length() == 0) { return new ScalarAnalysis(scalar, true, false, false, true, true, false); }
	...
	void writeTagDirective(String handleText, String prefixText) throws IOException {
		// : not sure 4 invocations better then StringBuilders created by str
		// + str
		stream.write("%TAG ");
